...
export default function Programmers() {
  ...
  const [featureProgrammerId, setFeatureProgrammarId] = useState(null)
    ...
    const getNameOfFeatured = () => {
      ...
      const progammerPick = featureProgrammerId === null ? null : programmersList.find(choice => {
        return featureProgrammerId === choice.id
    }) 
        return progammerPick.name
    };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featureProgrammerId === null ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeatureProgrammarId(dev.id)}}   >Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          featureProgrammerId !== null  ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳`
